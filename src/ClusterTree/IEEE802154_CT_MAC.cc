...
MAC_PIB IEEE802154_CT_MAC::MPIB =
{
...
UINT_16 IEEE802154_CT_MAC::addrCount = 0;

Define_Module(IEEE802154_CT_MAC);

IEEE802154_CT_MAC::IEEE802154_CT_MAC()
{
...
void IEEE802154_CT_MAC::registerInterface()
{
    iface=NULL;
...
/** Initialization */
void IEEE802154_CT_MAC::initialize(int stage)
{
    cSimpleModule::initialize(stage);
    EV << getParentModule()->getParentModule()->getFullName() << ": initializing IEEE802154_CT_MAC, stage=" << stage << endl;
    ...
    else if (2 == stage)
    {
        ...
        else if (!isCoordinator)    // start a device
            startDevice();
    }
}

void IEEE802154_CT_MAC::initializeQueueModule()
{
...
void IEEE802154_CT_MAC::receiveChangeNotification(int category, const cPolymorphic *details)
{
    Enter_Method_Silent();
...
/*************************** <Start star topology > *****************************/
//------------------------------------------------------------------------------------------------------------/
void IEEE802154_CT_MAC::startPANCoor()
{
...
void IEEE802154_CT_MAC::startDevice()
{
...
void IEEE802154_CT_MAC::startCoordinator(simtime_t now)
{
    mpib.macAssociationPermit = true;
    txSfSlotDuration = aBaseSlotDuration * (1 << mpib.macSuperframeOrder);
    startBcnTxTimer(true, now + beaconOffset);
}

//-------------------------------------------------------------------------------/
/*************************** <General Msg Handler> ******************************/
//-------------------------------------------------------------------------------/
void IEEE802154_CT_MAC::handleMessage(cMessage* msg)
{
...
void IEEE802154_CT_MAC::handleUpperMsg(cMessage* msg)
{
    uint16_t index;
    IE3ADDR destAddr;
    bool gtsFound = false;
    ...
    else        //  pkt destined for device
    {
        ...
        IEEE802154_CT_MAC* macModule = check_and_cast<IEEE802154_CT_MAC *>(module);
...
void IEEE802154_CT_MAC::handleLowerMsg(cMessage* msg)
{
    bool noAck;
    int i;
...
void IEEE802154_CT_MAC::handleSelfMsg(cMessage* msg)
{
    switch (msg->getKind())
    {
...
void IEEE802154_CT_MAC::sendDown(Ieee802154Frame* frame)
{
...
void IEEE802154_CT_MAC::reqtMsgFromIFq()
{
    if (queueModule)
    {
...
//-------------------------------------------------------------------------------/
void IEEE802154_CT_MAC::handleBeacon(Ieee802154Frame* frame)
{
    ...
    if (notAssociated) // this is my first rxed beacon, associate with this one,
    {
        ...
        EV << "Is Coordinator? " << isCoordinator << endl;
        if ((mpib.macBeaconOrder != 15) && isCoordinator)
        {
            EV << "Coordinator true! Sending beacon...." << endl;
            // actually started by mlme-start.request
            startCoordinator(now);
        }
...
void IEEE802154_CT_MAC::handleData(Ieee802154Frame* frame)
{
    FrameCtrl frmCtrl = frame->getFrmCtrl();
    bool isSIFS = false;
...
void IEEE802154_CT_MAC::handleAck(Ieee802154Frame* frame)
{
...
void IEEE802154_CT_MAC::handleCommand(Ieee802154Frame* frame)
{
...
bool IEEE802154_CT_MAC::frameFilter(Ieee802154Frame* frame)
...
void IEEE802154_CT_MAC::constructACK(Ieee802154Frame* rxFrame)
...
/***************************** <Radio State Control> ****************************/
//-------------------------------------------------------------------------------/
void IEEE802154_CT_MAC::resetTRX()
{
    PHYenum t_state;
...
/**************************** <MAC2PHY primitive> *******************************/
//-------------------------------------------------------------------------------/
void IEEE802154_CT_MAC::PLME_SET_TRX_STATE_request(PHYenum state)
{
...
void IEEE802154_CT_MAC::PLME_SET_request(PHYPIBenum attribute)
{
...
void IEEE802154_CT_MAC::PLME_CCA_request()
{
...
void IEEE802154_CT_MAC::PLME_bitrate_request()
{
    // construct PLME_CCA_request primitive
    Ieee802154MacPhyPrimitives *primitive = new Ieee802154MacPhyPrimitives();
...
//-------------------------------------------------------------------------------/
void IEEE802154_CT_MAC::handleMacPhyPrimitive(int msgkind, cMessage* msg)
{
...
void IEEE802154_CT_MAC::handle_PD_DATA_confirm(PHYenum status)
{
    inTransmission = false;
...
void IEEE802154_CT_MAC::handle_PLME_CCA_confirm(PHYenum status)
{
...
void IEEE802154_CT_MAC::handle_PLME_SET_TRX_STATE_confirm(PHYenum status)
{
...
/***************************** <SSCS-MAC Primitive> *****************************/
//-------------------------------------------------------------------------------/
void IEEE802154_CT_MAC::MCPS_DATA_request(uint16_t SrcAddrMode, UINT_16 SrcPANId, IE3ADDR SrcAddr,
...
void IEEE802154_CT_MAC::MCPS_DATA_confirm(MACenum status)
{
    // should report to upper layer here
    return;
}

void IEEE802154_CT_MAC::MCPS_DATA_indication(Ieee802154Frame* tmpData)
{
    if (tmpData->getIsGTS())
    {
        numRxGTSPkt++;
...
/***************************** <CSMA/CA Functions> ******************************/
//-------------------------------------------------------------------------------/
void IEEE802154_CT_MAC::csmacaEntry(char pktType)
{
...
void IEEE802154_CT_MAC::csmacaResume()
{
    bool ackReq;
...
void IEEE802154_CT_MAC::csmacaStart(bool firsttime, Ieee802154Frame* frame, bool ackReq)
{
    bool backoff;
...
void IEEE802154_CT_MAC::csmacaCancel()
{
...
void IEEE802154_CT_MAC::csmacaCallBack(PHYenum status)
{
    if (((!txBcnCmd)||(waitBcnCmdAck))
...
void IEEE802154_CT_MAC::csmacaReset(bool bcnEnabled)
{
    if (bcnEnabled)
    {
        NB = 0;
...
simtime_t IEEE802154_CT_MAC::csmacaAdjustTime(simtime_t wtime)
{
...
simtime_t IEEE802154_CT_MAC::csmacaLocateBoundary(bool toParent, simtime_t wtime)
{
    int align;
...
bool IEEE802154_CT_MAC::csmacaCanProceed(simtime_t wtime, bool afterCCA)
{
    bool ok;
...
simtime_t IEEE802154_CT_MAC::getFinalCAP(char trxType)
{
...
void IEEE802154_CT_MAC::csmaca_handle_RX_ON_confirm(PHYenum status)
{
    simtime_t now, wtime;
...
void IEEE802154_CT_MAC::csmacaTrxBeacon(char trx)
...
/***************************** <Timer starter> **********************************/
//-------------------------------------------------------------------------------/
void IEEE802154_CT_MAC::startBackoffTimer(simtime_t wtime)
{
...
void IEEE802154_CT_MAC::startDeferCCATimer(simtime_t wtime)
{
...
void IEEE802154_CT_MAC::startBcnRxTimer()
{
...
void IEEE802154_CT_MAC::startBcnTxTimer(bool txFirstBcn, simtime_t startTime)
{
...
void IEEE802154_CT_MAC::startAckTimeoutTimer()
{
...
void IEEE802154_CT_MAC::startTxAckBoundTimer(simtime_t wtime)
{
...
void IEEE802154_CT_MAC::startTxCmdDataBoundTimer(simtime_t wtime)
{
...
void IEEE802154_CT_MAC::startIfsTimer(bool sifs)
{
    simtime_t wtime;
    if (sifs)                   // SIFS
...
void IEEE802154_CT_MAC::startTxSDTimer()
...
void IEEE802154_CT_MAC::startRxSDTimer()
...
/******************************* <Timer Handler> ********************************/
//--------------------------------------------------------------------------------/
void IEEE802154_CT_MAC::handleBackoffTimer()
{
...
void IEEE802154_CT_MAC::handleDeferCCATimer()
{
    taskP.taskStatus(TP_CCA_CSMACA) = true;
    EV << "[MAC-TASK]: TP_CCA_CSMACA = true" << endl;
    PLME_CCA_request();
...
void IEEE802154_CT_MAC::handleBcnRxTimer()
{
...
void IEEE802154_CT_MAC::handleBcnTxTimer()
{
    FrameCtrl frmCtrl;
    /*TRANSACLINK *tmp;
    int i;*/
...
void IEEE802154_CT_MAC::handleAckTimeoutTimer()
{
...
void IEEE802154_CT_MAC::handleTxAckBoundTimer()
{
    if (!beaconWaitingTx)
...
void IEEE802154_CT_MAC::handleTxCmdDataBoundTimer()
...
void IEEE802154_CT_MAC::handleIfsTimer()
...
void IEEE802154_CT_MAC::handleSDTimer()     // we must make sure that outgoing CAP and incoming CAP never overlap
{
    inTxSD_txSDTimer = false;
    inRxSD_rxSDTimer = false;
...
//-------------------------------------------------------------------------------/
void IEEE802154_CT_MAC::taskSuccess(char type, bool csmacaRes)
{
    UINT_16 t_CAP;
    uint16_t ifs;
    simtime_t tmpf;
...
void IEEE802154_CT_MAC::taskFailed(char type, MACenum status, bool csmacaRes)
{
    if ((type == 'b')   //beacon
            || (type == 'a')    //ack.
            || (type == 'c'))   //command
...
void IEEE802154_CT_MAC::checkTaskOverflow(Ieee802154MacTaskType task)
...
void IEEE802154_CT_MAC::FSM_MCPS_DATA_request(PHYenum pStatus, MACenum mStatus)
{
    Ieee802154MacTaskType task = TP_MCPS_DATA_REQUEST;
...
void IEEE802154_CT_MAC::dispatch(PHYenum pStatus, const char *frFunc, PHYenum req_state, MACenum mStatus)
{
    FrameCtrl frmCtrl;
    bool isSIFS = false;
    //int i;
...
/****************************** <Utility Functions> *****************************/
//-------------------------------------------------------------------------------/
int IEEE802154_CT_MAC::calFrmByteLength(Ieee802154Frame* frame)
{
...
int IEEE802154_CT_MAC::calMHRByteLength(uint16_t addrModeSum)
{
    switch (addrModeSum)
    {
    case 0:
...
simtime_t IEEE802154_CT_MAC::calDuration(Ieee802154Frame* frame)
{
    return (def_phyHeaderLength*8 + frame->getBitLength())/phy_bitrate;
}

double IEEE802154_CT_MAC::getRate(char bitOrSymbol)
{
    double rate;
...
bool IEEE802154_CT_MAC::toParent(Ieee802154Frame* frame)
{
 ...
 */
void IEEE802154_CT_MAC::gtsScheduler()
{
//    uint16_t t_SO;
...
void IEEE802154_CT_MAC::startGtsTimer(simtime_t w_time)
{
...
void IEEE802154_CT_MAC::handleGtsTimer()
{
    simtime_t w_time;
 ...
 */
uint16_t IEEE802154_CT_MAC::gts_request_cmd(UINT_16 macShortAddr, uint16_t length, bool isReceive)
{
    Enter_Method_Silent();
    uint16_t startSlot;
    UINT_32 t_cap;
...
void IEEE802154_CT_MAC::startFinalCapTimer(simtime_t w_time)
{
...
void IEEE802154_CT_MAC::handleFinalCapTimer()
{
...
UINT_16 IEEE802154_CT_MAC::associate_request_cmd(IE3ADDR extAddr, const DevCapability& capaInfo)
{
    Enter_Method_Silent();
 ...
 */
bool IEEE802154_CT_MAC::gtsCanProceed()
{
    simtime_t t_left;
...
MACAddress IEEE802154_CT_MAC::getLongAddress(UINT_16 val)
{
    MACAddress tmp;
