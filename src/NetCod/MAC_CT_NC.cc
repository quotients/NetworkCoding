...
void MAC_CT_NC::registerInterface()
{
    iface=NULL;
...
/** Initialization */
void MAC_CT_NC::initialize(int stage)
{
    cSimpleModule::initialize(stage);
    EV << getParentModule()->getParentModule()->getFullName() << ": initializing MAC_CT_NC, stage=" << stage << endl;
...
void MAC_CT_NC::initializeQueueModule()
{
...
void MAC_CT_NC::receiveChangeNotification(int category, const cPolymorphic *details)
{
    Enter_Method_Silent();
...
void MAC_CT_NC::startCoordinator(simtime_t now)
{
    mpib.macAssociationPermit = true;
    txSfSlotDuration = aBaseSlotDuration * (1 << mpib.macSuperframeOrder);
...
/*************************** <General Msg Handler> ******************************/
//-------------------------------------------------------------------------------/
void MAC_CT_NC::handleMessage(cMessage* msg)
{
...
void MAC_CT_NC::handleUpperMsg(cMessage* msg)
{
    uint16_t index;
    IE3ADDR destAddr;
    bool gtsFound = false;
    ...
    else        //  pkt destined for device
    {
        ...
        MAC_CT_NC* macModule = check_and_cast<MAC_CT_NC *>(module);
...
void MAC_CT_NC::handleLowerMsg(cMessage* msg)
{
    bool noAck;
    int i;
...
void MAC_CT_NC::handleSelfMsg(cMessage* msg)
{
    switch (msg->getKind())
    {
...
void MAC_CT_NC::sendDown(Ieee802154Frame* frame)
{
...
//-------------------------------------------------------------------------------/
void MAC_CT_NC::handleBeacon(Ieee802154Frame* frame)
{
...
void MAC_CT_NC::handleData(Ieee802154Frame* frame)
{
    FrameCtrl frmCtrl = frame->getFrmCtrl();
    bool isSIFS = false;
...
void MAC_CT_NC::handleAck(Ieee802154Frame* frame)
{
...
void MAC_CT_NC::handleCommand(Ieee802154Frame* frame)
{
...
bool MAC_CT_NC::frameFilter(Ieee802154Frame* frame)
...
void MAC_CT_NC::constructACK(Ieee802154Frame* rxFrame)
...
/**************************** <MAC2PHY primitive> *******************************/
//-------------------------------------------------------------------------------/
void MAC_CT_NC::PLME_SET_TRX_STATE_request(PHYenum state)
{
...
void MAC_CT_NC::PLME_SET_request(PHYPIBenum attribute)
{
...
void MAC_CT_NC::PLME_CCA_request()
{
...
void MAC_CT_NC::PLME_bitrate_request()
{
    // construct PLME_CCA_request primitive
    Ieee802154MacPhyPrimitives *primitive = new Ieee802154MacPhyPrimitives();
...
//-------------------------------------------------------------------------------/
void MAC_CT_NC::handleMacPhyPrimitive(int msgkind, cMessage* msg)
{
...
void MAC_CT_NC::handle_PD_DATA_confirm(PHYenum status)
{
    inTransmission = false;
...
void MAC_CT_NC::handle_PLME_CCA_confirm(PHYenum status)
{
...
void MAC_CT_NC::handle_PLME_SET_TRX_STATE_confirm(PHYenum status)
{
...
/***************************** <SSCS-MAC Primitive> *****************************/
//-------------------------------------------------------------------------------/
void MAC_CT_NC::MCPS_DATA_request(uint16_t SrcAddrMode, UINT_16 SrcPANId, IE3ADDR SrcAddr,
...
void MAC_CT_NC::MCPS_DATA_confirm(MACenum status)
{
    // should report to upper layer here
    return;
}

void MAC_CT_NC::MCPS_DATA_indication(Ieee802154Frame* tmpData)
{
    if (tmpData->getIsGTS())
    {
        numRxGTSPkt++;
...
/***************************** <CSMA/CA Functions> ******************************/
//-------------------------------------------------------------------------------/
void MAC_CT_NC::csmacaEntry(char pktType)
{
...
void MAC_CT_NC::csmacaStart(bool firsttime, Ieee802154Frame* frame, bool ackReq)
{
    bool backoff;
...
void MAC_CT_NC::csmacaCallBack(PHYenum status)
{
    if (((!txBcnCmd)||(waitBcnCmdAck))
...
void MAC_CT_NC::csmacaReset(bool bcnEnabled)
{
    if (bcnEnabled)
    {
        NB = 0;
...
simtime_t MAC_CT_NC::csmacaAdjustTime(simtime_t wtime)
{
...
simtime_t MAC_CT_NC::csmacaLocateBoundary(bool toParent, simtime_t wtime)
{
    int align;
...
bool MAC_CT_NC::csmacaCanProceed(simtime_t wtime, bool afterCCA)
{
    bool ok;
...
simtime_t MAC_CT_NC::getFinalCAP(char trxType)
{
...
void MAC_CT_NC::csmaca_handle_RX_ON_confirm(PHYenum status)
{
    simtime_t now, wtime;
...
void MAC_CT_NC::csmacaTrxBeacon(char trx)
...
/***************************** <Timer starter> **********************************/
//-------------------------------------------------------------------------------/
void MAC_CT_NC::startBackoffTimer(simtime_t wtime)
{
...
void MAC_CT_NC::startDeferCCATimer(simtime_t wtime)
{
...
void MAC_CT_NC::startBcnRxTimer()
{
...
void MAC_CT_NC::startBcnTxTimer(bool txFirstBcn, simtime_t startTime)
{
...
void MAC_CT_NC::startAckTimeoutTimer()
{
...
void MAC_CT_NC::startTxAckBoundTimer(simtime_t wtime)
{
...
void MAC_CT_NC::startTxCmdDataBoundTimer(simtime_t wtime)
{
...
void MAC_CT_NC::startIfsTimer(bool sifs)
{
    simtime_t wtime;
    if (sifs)                   // SIFS
...
/******************************* <Timer Handler> ********************************/
//--------------------------------------------------------------------------------/
void MAC_CT_NC::handleBackoffTimer()
{
...
void MAC_CT_NC::handleDeferCCATimer()
{
    taskP.taskStatus(TP_CCA_CSMACA) = true;
    EV << "[MAC-TASK]: TP_CCA_CSMACA = true" << endl;
    PLME_CCA_request();
...
void MAC_CT_NC::handleBcnRxTimer()
{
...
void MAC_CT_NC::handleBcnTxTimer()
{
    FrameCtrl frmCtrl;
    /*TRANSACLINK *tmp;
    int i;*/
...
void MAC_CT_NC::handleAckTimeoutTimer()
{
...
void MAC_CT_NC::handleTxAckBoundTimer()
{
    if (!beaconWaitingTx)
...
void MAC_CT_NC::handleTxCmdDataBoundTimer()
...
void MAC_CT_NC::handleSDTimer()     // we must make sure that outgoing CAP and incoming CAP never overlap
{
    inTxSD_txSDTimer = false;
    inRxSD_rxSDTimer = false;
...
//-------------------------------------------------------------------------------/
void MAC_CT_NC::taskSuccess(char type, bool csmacaRes)
{
    UINT_16 t_CAP;
    uint16_t ifs;
    simtime_t tmpf;
...
void MAC_CT_NC::taskFailed(char type, MACenum status, bool csmacaRes)
{
    if ((type == 'b')   //beacon
            || (type == 'a')    //ack.
            || (type == 'c'))   //command
...
void MAC_CT_NC::checkTaskOverflow(Ieee802154MacTaskType task)
...
void MAC_CT_NC::FSM_MCPS_DATA_request(PHYenum pStatus, MACenum mStatus)
{
    Ieee802154MacTaskType task = TP_MCPS_DATA_REQUEST;
...
void MAC_CT_NC::dispatch(PHYenum pStatus, const char *frFunc, PHYenum req_state, MACenum mStatus)
{
    FrameCtrl frmCtrl;
    bool isSIFS = false;
    //int i;
...
/****************************** <Utility Functions> *****************************/
//-------------------------------------------------------------------------------/
int MAC_CT_NC::calFrmByteLength(Ieee802154Frame* frame)
{
...
int MAC_CT_NC::calMHRByteLength(uint16_t addrModeSum)
{
    switch (addrModeSum)
    {
    case 0:
...
simtime_t MAC_CT_NC::calDuration(Ieee802154Frame* frame)
{
    return (def_phyHeaderLength*8 + frame->getBitLength())/phy_bitrate;
}

double MAC_CT_NC::getRate(char bitOrSymbol)
{
    double rate;
...
bool MAC_CT_NC::toParent(Ieee802154Frame* frame)
{
...
void MAC_CT_NC::startGtsTimer(simtime_t w_time)
{
 ...
 */
uint16_t MAC_CT_NC::gts_request_cmd(UINT_16 macShortAddr, uint16_t length, bool isReceive)
{
    Enter_Method_Silent();
    uint16_t startSlot;
    UINT_32 t_cap;
...
void MAC_CT_NC::startFinalCapTimer(simtime_t w_time)
{
...
void MAC_CT_NC::handleFinalCapTimer()
{
...
UINT_16 MAC_CT_NC::associate_request_cmd(IE3ADDR extAddr, const DevCapability& capaInfo)
{
    Enter_Method_Silent();
...
MACAddress MAC_CT_NC::getLongAddress(UINT_16 val)
{
    MACAddress tmp;
